...
ax1.scatter(dataset[label==1,0],dataset[label==1,1],c='g',s=30,label='good')
ax1.scatter(dataset[label==0,0],dataset[label==0,1],c='k',s=30,label='bad')
...
for i in range(m):
    ...
    y1 = k * x0
    if label[i]==1:
        ax1.scatter(x1,y1,c='g',s=30)
        ax1.plot([x0,x1],[y0,y1],'g--')
    else:
        ax1.scatter(x1, y1, c='k',s=30)
